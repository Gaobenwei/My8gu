...
![image-20241030114620850](./png/image-20241030114620850.png)

叉乘是一个**向量运算**，结果是一个新向量，它与原来的两个向量都垂直。

![image-20241030114703040](./png/image-20241030114703040.png)
...
> 注意，区间有序**关键在于和比较函数或方法密切相关**，默认如果是递增，相当于：upper_bound( begin , end , val , less<type>() ),这里升序，输出第一个后序即（判断为true，就是第一个 value<elem 的elem）。
>
> 而递减序列，在排序的时候可知sort(,,greater<type>()),那么：upper_bound( begin , end , val , greater<type>() )，输出第一个后序（即>的判断为true时为 value>elem 的elem）的数。
